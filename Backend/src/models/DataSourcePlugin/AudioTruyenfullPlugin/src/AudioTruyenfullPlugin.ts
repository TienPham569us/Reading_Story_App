...
export default class AudioTruyenfullPlugin implements IDataSourcePlugin {
  name: string;
  ...
  static baseUrl: string = 'https://truyenaudiohay.net'; //'https://audiotruyenfull.com';
  public constructor(
    name: string,
    listCategory: ICategoryList,
    contentChapter: IContentStory,
  ...
  clone(name: string): IDataSourcePlugin {
    return new AudioTruyenfullPlugin(
      this.name,
  ...
  getBaseUrl(): string {
    return AudioTruyenfullPlugin.baseUrl;
  }
  public async search(
    title: string,
